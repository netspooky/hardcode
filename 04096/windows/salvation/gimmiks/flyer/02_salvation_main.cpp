...
//cube zonly
// cool
"float4 PSTD(vso i):COLOR0"
"{"
  "return i.ps.z/i.ps.w;"
"}"
"technique"
"{"
...
D3DXVECTOR4 lightpos(-150,180,-50,0);
...
static float bias[16] = {	
	0.49995f, 0.0, 0.0, 0.0, 
	0.0, -0.49995f, 0.0, 0.0,
	0.0, 0.0, 0.49995f, 0.0,
	0.5f, 0.5f, 0.5f, 1.0f};
static float sinv[8]={-1,0,1,1,1,0,-1,-1};
static float cosv[8]={1,1,1,0,-1,-1,-1,0};

static float Sphere_Colors[] = {COL(0,0,0,0), COL(0.6f,0.5f,0.4f,0.f)};
...
void __fastcall PlaceSphere(int type)
{
  ...
  {
    float lerp = frand_4k() * 0.25f + 0.5f;
    *(c+0) = lerp * 0.3f;
    *(c+1) = lerp * 0.6f;
    *(c+2) = lerp * 0.4f;
  }
...
float nx,ny,nz;
void __fastcall VertexCube(float xa,float ya,float za)
{
  float p[4];
  float c[4];
  p[0]=sphere_p[0]+xa;
...
void __fastcall DrawObject(int nr,int pass)
{
  ...
  effect->Begin(&passCount,0);//D3DXFX_DONOTSAVESTATE|D3DXFX_DONOTSAVESAMPLERSTATE|D3DXFX_DONOTSAVESHADERSTATE);
...
void __fastcall CompileObject(int nr)
{
  ...
  d3dDevice->CreateVertexBuffer(currentVertex*VERTEX_SIZE_IN_FLOATS*sizeof(float),D3DUSAGE_WRITEONLY,VERTEX_FORMAT,D3DPOOL_DEFAULT,&o,0);
...
void __fastcall PrepareMatrices(D3DXMATRIX &camView)
{
  ...
  //lightdir.w = 0;
  effect->SetVector(HANDLE_B,&lightpos);
...
void __fastcall Draw_Scene(int pass)
{
  ...
  Translation.y=0;
  Translation.z=0;
  static const int L1[2]={PASS_ZRENDER,PASS_STONY};
  int p3=L1[pass];
  DrawObject(OBJECT_GROUND,p3);
  static const int L2[2]={PASS_CUBE_ZRENDER,PASS_CUBE};
  int p2=L2[pass];
  ...
  for (int x=0;x<TOWER_COUNT1;++x)
  {
    ...
    if ((Translation.y > -58.f)&&(((Translation.x<-30)&&(Translation.z<100))||((Translation.x>100))))
    {
      DrawObject(geb,p2);
      Translation = Translation+TowerHighest[geb-OBJECT_TOWER_START];
      if (Translation.x > -75.f&&Translation.x < 25.f)
...
#endif
{
  ...
  // COMPILE SHADERS
  // ---------------------------------------------
//#define SAVESTATE (D3DXFX_DONOTSAVESTATE|D3DXFX_DONOTSAVESHADERSTATE|D3DXFX_DONOTSAVESAMPLERSTATE|D3DXSHADER_PREFER_FLOW_CONTROL)
#define SAVESTATE (D3DXSHADER_PREFER_FLOW_CONTROL)
  ...
  d3dDevice->CreateDepthStencilSurface(SHADOWMAP_WIDTH,SHADOWMAP_HEIGHT, D3DFMT_D24S8,MULTISAMPLE,0,1,&newDepthBuffer,0);
  ...
  // ---------------------------------------------
  for (float x = -825.f; x < 512.f; x += 7.5f)
  for (float z = -825.f; z < 825.f; z += 7.5f)
  {
    float sq=x*x+z*z;
    sphere_p[0] = x;
    sphere_p[1] = sin(sq*0.00025f)*30.f;
    sphere_p[2] = z;
    //sphere_s = 0.75f;
    if (sphere_p[1]>0&&sq>40000.f)
...
#define A (20+xf)
  for (float xf = 0.f; xf < 60; xf+=30.f)
  {
    ...
    CUBE(-9+A,40.5f,-50,1,1,700);
    CUBE(+8+A,40.5f,-50,1,1,700);
    for (float x=-700+xf;x<700;x+=20)
    {
      CUBE(0+A,20,x,1,20,1);
      CUBE(-9+A,47,x,.25f,7,.25f);
      CUBE(+9+A,47,x,.25f,7,.25f);
      CUBE(0+A,54,x,9,.25f,.25f);
    }
  }
  ...
  for (int x=OBJECT_TOWER_START;x<=OBJECT_TOWER_END;++x)
  {
    ...
    for (int i =0; i < 15; ++i)
    {
      ...
      float z=frand_4k()*20.f;
      float xp=frand_4k()*20.f;
      CUBE(xp,f,z,5,f,5,1);
  ...
  //HMIDIOUT a;
  //midiOutOpen(&a,0,0,0,0);
  //midiOutShortMsg(a,0x000015c0); // 11 15
  //midiOutShortMsg(a,0x7f1090);
  //midiOutShortMsg(a,0x7f1790);
  ...
  do
  {
    ...
    const int td = (timeGetTime() - startTime);
    const float sinofs = sin(td*0.000085f)*4.f;
    char *c = &CamPosStartEnd[((td>>13) & (ANZCAM-1))*3];
    for (int i = 0;i < 3; ++i)
    {
      campos[i] = (*(c+ANZCAM*3)*sinofs+*(c)*2.f);c++;
    }
    ...
    LookAt(&lightViewMatrix, (D3DXVECTOR3*)&lightpos,&lightfocuspoint,&up);
    ...
    DrawObject(OBJECT_WATER,PASS_WATER);
