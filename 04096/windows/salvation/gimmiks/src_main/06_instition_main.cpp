...
"float4 tf(float i)"
"{"
  "float4 ret=tex1Dlod(txns,float4(i,0,0,0));"
...
"float rmf(float4 PP,float o=20,float H=.8,float l=1.875,float of=.75,float sh=1.4,float th=5,float Kt=.9)"
"{"
  "PP.xz*=4;PP.x+=7;PP.z+=6;"
	"float r,s,i;"
	"for(i=0;i<o;i+=1)"
  "{"
    "if(i==0)" 
    "{"
      "s=pow(abs(of-abs(snoise(PP))),sh);"
      "r=s;"
    "}"
    "else"
    "{"
      "PP*=l;"
      "s=pow(abs(of-abs(snoise(PP))),sh)*clamp(s*th,0,1);"
   		"r+=s*pow(l,(-i*H));"
    "}"
	"}"
  "return 1-r*Kt;"
"}"
"vso LOVS(float4 ps:POSITION,float2 tc:TEXCOORD0)"
"{"
  "vso o;o.psw=ps;o.cl=0;o.tc=tc;o.ps=0;o.pt=0;return o;"
"}"
//vertexshader0 sphere shader
...
"{"
  ...
  "o.cl=lerp(float4(.23,.23,.2,0),float4(.4,.4,.325,0),-c+1)+b.y*.001;"
  "o.dp=.9999;"
  "return o;"
"}"
//landscapeshader
...
"{"
  ...
  "float t1=rmf(ps,10);"
  "float t2=rmf(ps+float4(-2*.0002,0,0,0),10);"
  "float t3=rmf(ps+float4(0,0,-2*.0002,0),10);"
  "float4 sn=normalize(float4(s2-s1,.01,s3-s1,0));"
  "float4 tn=normalize(float4(t2-t1,.01,t3-t1,0));"
  "float4 c=lerp(float4(.10,.11,.12,0),float4(.22,.21,.20,0),clamp(dot(sn,normalize(float4(-1,1,-1,0))),0,1));"
  "c*=max(.5,dot(tn,normalize(b.xyz-i.ps.xyz))*2);"
  "c=lerp(float4(.2,.3,.1,0),c,clamp(.9+s1,0,1));"
...
"{"
  ...
  //"float4 pz=mul(i.pt,lvpm);pz/=pz.w;"
  "float4 c=max(0,dot(normalize(l),n))*.4+.2;"
...
"{"
  ...
  "rn=mul(mvm,rn);"
  "float4 c=max(0.2,dot(normalize(n),normalize(b))*2)*float4(.25,.25,.25,0)+max(0.2,-dot(normalize(n),normalize(b)))*float4(.3,.4,.25,0);"
  "o.cl=c;"
  "return o;"
"}"
"technique T0"
"{"
  ...
  "pass P3"
  "{"
    "VertexShader=compile vs_3_0 LOVS();"
...
#define PASS_SKY 0
#define PASS_LANDSCAPE 1
#define PASS_CUBE 2 
#define PASS_RIDGED 3
#define PASS_SPHERE 4
...
D3DXVECTOR3 lightpos(0,180,0);
D3DXVECTOR3 lightfocuspoint(-800,0,0);
D3DXVECTOR3 campos(0,50,20);
...
void __fastcall Draw_Scene(int pass)
{
  ...
  d3dDevice->BeginScene();
  DrawObject(OBJECT_LSCAPE,PASS_LANDSCAPE);
  DrawObject(OBJECT_CUBES,PASS_CUBE);
  Translation.x=-200;
  Translation.y=200;
  Translation.z=-300;
  DrawObject(OBJECT_FUTURA,PASS_SPHERE);
  d3dDevice->EndScene();
}
...
#endif
{
  ...
  // COMPILE SHADERS
  // ---------------------------------------------
#define SAVESTATE D3DXSHADER_PREFER_FLOW_CONTROL|D3DXSHADER_OPTIMIZATION_LEVEL0
#ifndef CHECKEFFECT
...
#define dx 5.f
#define dz 5.f
  for (float x=-1400.f;x < 0.f; x+=dx)
  {
    for (float z=-1400.f;z < 200.f; z+=dz)
    {
      const float x2= x+dx;
      const float z2= z+dz;
      PlaceLandscape(x,z);
  ...
  {
    sphere_p[0]=frand_4k()*500.f-500.f;
    sphere_p[2]=frand_4k()*8.5f+22.5f;
    GetHeight(sphere_p);
    sphere_p[1]*=0.5f;
    sphere_p[1]+=71.5f+frand_4k()*2.f;
    cube_s[0]=0.4f;
    cube_s[1]=0.1f;
    cube_s[2]=0.4f;
    PlaceCube();
  ...
  do
  {
    ...
    // ---------------------------------------------
    Frustum(&cameraProjectionMatrix, CAMFOV/360.f*2.f*3.1416f, ASPECT, 0.9f, 1800 );
