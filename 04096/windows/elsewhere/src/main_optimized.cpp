...
static const char *shaderCode[SHADER_COUNT]=
{
  ...
  "{"
    ...
    "for(k=0.;k<=5.;k+=1.)"
    "{"
      "w=.39895*exp(k*k*-.02);"//( 1 / ( sqrt(2*pi) * sigma ) ) * exp( -(x*x) / (2*sigma*sigma) ); -> sigma = 1.0 and x = k / 5.
      "r=c+o*k;"
      "if(clamp(r,0.,1.)==r)"
      "{"
        "z+=texture2D(t,r)*w;"
...
#define tyres yres
  ...
  {		
    ...
    frameTimeMorphBefore = _floorf2(currentFrameTime / currentMorphDelta);
    frameTimeMorphAfter = frameTimeMorphBefore + 1.f;
    frameTimeMorphBefore *= currentMorphDelta;
    frameTimeMorphAfter *= currentMorphDelta;
    ...
    static int bugDisabler = 0; bugDisabler++; // this bug in the optimized version is strange :)
    if (morphNextFrameSave[MAX_MORPH_OBJECTS * 4 * 4-1] != frameTimeMorphAfter || bugDisabler < 32)
    {
      morphNextFrameSave[MAX_MORPH_OBJECTS * 4 * 4-1] = frameTimeMorphAfter;
