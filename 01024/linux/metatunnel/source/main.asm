...
db	"vec2 v=(gl_FragCoord.xy-vec2(400,300))/vec2(400,300);"
db	"float w=dot(gl_Color.xyz,vec3(1,256,65536))*.25;"
db	"const float s=0.4;"

db	"float obj(vec3 pos){"
db		"float final=1.0;"
db		"final*=distance(pos,vec3(cos(w)+sin(w*0.2),0.3,2.0+cos(w*0.5)*0.5));"
db		"final*=distance(pos,vec3(-cos(w*0.7),0.3,2.0+sin(w*0.5)));"
db		"final*=distance(pos,vec3(-sin(w*0.2)*0.5,sin(w),2.0));"
db		"final *=cos(pos.y)*cos(pos.x)-0.1-cos(pos.z*7.+w*7.)*cos(pos.x*3)*cos(pos.y*4.)*0.1;"
db		"return final;"
db	"}"

db	"void main(){vec3 o=vec3(v.x,v.y*1.25,0.0);"
db		"vec3 d=vec3(v.x+cos(w)*.3,v.y,1.0)/64.0;"
db		"vec4 color=vec4(0.0);"
db		"float t=0.0;"
db		"for(int i=0;i<75;i++) {"
db			"if(obj(o+d*t)<s){"
db				"t-=5.0;"
db				"for(int j=0;j<5;j++){"
db					"if(obj(o+d*t)<s){"
db						"break;"
db					"}"
db					"t+=1.0;"
db				"}"
db				"vec3 e=vec3(0.01,.0,.0);"
db				"vec3 n=vec3(0.0);"
db				"n.x=obj(o+d*t)-obj(vec3(o+d*t+e.xyy));"
db				"n.y=obj(o+d*t)-obj(vec3(o+d*t+e.yxy));"
db				"n.z=obj(o+d*t)-obj(vec3(o+d*t+e.yyx));"
db				"n=normalize(n);"
db				"color+=max(dot(vec3(0.0,0.0,-0.5),n),0.0)+max(dot(vec3(0.0,-0.5,0.5),n),0.0)*0.5;break;"
db			"}"
db			"t+=5.0;"
db		"}"
db		"gl_FragColor=color+vec4(0.1,0.2,0.5,1.0)*(t*0.025);"
db	"}"
db 0 
