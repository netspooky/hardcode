; ------------------------------------------------------
; 1kPack v0.9c - Depacker (QueryPerformanceCounter version)
; Written by Franck "hitchhikr" Charlet / Neural
; ------------------------------------------------------
; buildblock RELEASE
; CAPT "[SOURCEDIR]..\nasmw.exe" -f bin "%2" -o "%1.bin" -s -O9
; buildblockend

; !!! ONLY VALID FOR D3DX9_30.DLL !!!
; You'll have to modify this if you want to use another d3dx9 dll
...
do_loop:								loop	set_addresses_loop				; 64 (2) msg CheckSum
                                        ...
                                        db      0,0                             ; 74
										lea		eax, [ebx + (0xb0 - 0x4c)]		; 76 (3)
										push	0x38							; 79 (2) z_stream_s structure size
										push	eax								; 7b (1) version
										push	ebx								; 7c (1) z_stream_s
										jmp		depacker						; 7e (2) depack it SizeOfHeapCommit
										db		0						        ; 7d data_type 
...
_fOpt_Header:
										...
										dd		0x00010000						; a8
										dd		0x00000200						; ac
										dd		0x00000031						; b0 also used as ZLIB_VERSION (only the first char)
										dd		D3DX_LoadLibrary				; b4
										dd		D3DX_GetProcAddress				; b8
										dd		D3DX_GetTickCount           	; bc
depacker:
