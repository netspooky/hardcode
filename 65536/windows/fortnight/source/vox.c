...
static __inline int lrint(float a) 
{ 
	int bla; 
	__asm { 
		fld dword ptr [a] 
...
static __inline float tsox_gv(Tso *t, int lkm, float x, float y, float z) { 
...
static __inline float tsox_gv2(Tso *t, int lkm, int x, int y, int z) { 
  float v=0, vv, a; int i;
  unsigned ia;
  for (i=0; i<lkm; i++, t++) { 
...
static __inline float tsox_grad(Tso *t, int lkm, float x, float y, float z, float xd, float yd, float zd) {
  float a; int i;
  unsigned ia;
...
static void rend(unsigned char *buf, int pit, int xs, int ys, Tso *t, int cnt) {
  Tso *tt;
