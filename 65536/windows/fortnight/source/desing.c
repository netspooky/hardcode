...
__inline int yrand(int size, int seed) { unsigned a, b; a=seed*1507715517+(rando>>3)*0x23f321c7^seed*0xdeadbeef; b=a*231554621^a*876423437; return (b>>16)%size; }
__inline int xrand(int size, int seed) {
  int a=yrand(size, seed-1), b=yrand(size, seed);
  if (a==b) return yrand(size, seed*3); else return b;
}


__inline float zap(float x, float z) { return x<0?0:1./(x+z); }
//
...
*/
  ...
  if(xrand(10,phi)<2) {
    duunaa_kirkkovene(buf, buf, pitch, xs, ys, -kont, bri+1, .1f, 0, 0);
  } else {
    duunaa_kirkkovene(buf, buf, pitch, xs, ys, kont, bri, .1f, 0, 0);
  }
