...
static int dx;
static int dy;
static int true_8bit = 0; // don't use convert() in 8bit modes, and don't try setting the palette.
...
static void ptc_paint_primary()
{
    ...
    if (lpDDS)
    {
        ...
        IDirectDrawSurface_Blt(lpDDS,&destination,lpDDS_secondary,&source,0,0);
    }
}
...
int ptc_open(char *title,int width,int height)
{
    ...
    // enter display mode
    if (FAILED(IDirectDraw_SetDisplayMode(lpDD,width,height,8)))
    {
...
ptc_setpal(unsigned char * p)
{
  ...
  PALETTEENTRY ddpalentries[0x100]; 
  int i;
  ...
  if(true_8bit)
    {
      for(i=0; i<0x100; i++)
        {
          ddpalentries[i].peRed   = palette[i*3+0];
          ddpalentries[i].peGreen = palette[i*3+1];
          ddpalentries[i].peBlue  = palette[i*3+2];
        }

      if(FAILED(IDirectDraw_CreatePalette(lpDD, DDPCAPS_8BIT, ddpalentries, &ddpalette, NULL))) return 0;
      if(FAILED(IDirectDrawSurface_SetPalette(lpDDS, ddpalette))) return 0;
    }
