...
;www.sensenstahl.com
;fasm 1.69.31
;listening: undercover - imaginary friends 

;i wanted to make something with "cut" borders for a long time.
;i played around and  came  up with that city landscape and did
;not quite know what to do with it so i decided to grab the old
;idea and included it. and after thinking about it i also added
;some extra design on the facades of the buildings for no bytes
;extra. without them it looked so clean - too clean. now it  is
;somehow like color on a wall (at least in my mind ^^).  enjoy!
...
in al,40h
mul bx
shr al,2 ;prepare data for direct use to speed up things a little
inc ax   ;never 0
mov byte[gs:bx],al
inc bx
jnz set_map
...
inc bx
cmp bx,32;31 ;next tower on raster y
jna iso4
...
add bp,di
mov byte[fs:bp-320*6+9],ch ;top ;including adjust
neg bp
mov byte[fs:bp-4],ch    ;bottom ;including adjust
