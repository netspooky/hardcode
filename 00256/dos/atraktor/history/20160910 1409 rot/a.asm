...
; di=0(index into points)
  mov  [bp+si],sp
  fild   word[bp+si]
  fidiv  word[byte ss:si-100h+ROTSPEED] ; {t/ROTSPEED . . .}
  fsincos              ; {s c . . .}
  
D:
;rotate
  fild   word[di]      ; {x=p[i] s c . . .}
  fmul   st1           ; {sx s c . . .}
  fild   word[di]      ; {cx sx s c . . .}
  fmul   st3
  scasw
  fild   word[di]      ; {z=p[i+2] cx sx s c . . .}
  fmul   st3           ; {sz cx sx s c . . .}
  faddp  st1,st0       ; {sz+cx sx s c . . .}
  fild   word[di]      ; {z sz+cx sx s c . . .}
  fmul   st4           ; {cz sz+cx sx s c . . .}
  fsubrp st2,st0       ; {sz+cx cz-sx s c . . .}
  scasw
  fild   word[di]
  fxch   st1           ; {Z=sz+cx Y=y=p[i+4] X=cz-sx s c . . .}
...
;  fild   word[di]
  ...
  fiadd  word[ss:si]   ; {BIG+Z Y X s c . . .}
  fiadd  word[ss:si]   ; {2*BIG+Z Y X s c . . .}
  fidivr word[byte ss:si-100h+WIDTH]; {w=WIDTH/(Z+2*BIG) Y X s c . . .}
  
  fmul   st1,st0       ; {w Y*w X s c . . .}
  fmulp  st2,st0       ; {Y*w X*w s c . . .}
  fistp  word[bp+si]   ; Y*w {X*w s c . . .}
  imul bx,[bp+si],320  ; handle Y overflow
  fistp  word[bp+si]   ; X*w {s c . . .} but ignore X overflow
  jo   O
  add  bx,[bp+si]      
  add  bx,100*320+160  ; bx = adr = (iy+100)*320 + (ix+160)

  mov  al,[di+3]       ; (color=p[i+6]>>8): 0..255
  add  al,128
