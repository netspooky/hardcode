...
void iterate(void)
{
  long double d;
  dx += x; dy += y; dz += z;
  d = x*sin((A[2]*z + A[3]) * (1.0f/16384.0f)) + sin((A[0]*y + A[1])* (1.0f/16384.0f)); x = y; y = z; z = d;
  d = dx*sin((A[2]*dz + A[3])* (1.0f/16384.0f)) + sin((A[0]*dy + A[1])* (1.0f/16384.0f)); dx = dy; dy = dz; dz = d;
  dx -= x; dy -= y; dz -= z;
